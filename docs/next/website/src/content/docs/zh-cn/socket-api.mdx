...
| 领域 | 方法 |
| --- | --- |
| 服务器 | `ping`、`server.stop`、`server.reload_config`、`server.agent_manifests`、`server.reload_agent_manifests`、`server.dev_servers` |
...
`server.dev_servers` 列出由窗格进程树中的进程所拥有的 TCP 监听器 — 与 dev servers 窗口背后的数据相同。它没有参数。响应为 `type: "dev_server_list"`，带 `servers` 数组；每个条目报告 `pid`、`name`、由 `address`/`port` 对组成的 `listeners`，以及可选的 `command`、`uptime_seconds`、`pane_id`、`workspace_id`、`workspace_name`、`pane_title` 和 `cwd`。
...
`process.kill` 仅在 pid 位于窗格进程树内时将其终止。作用域在每个请求时重新计算，位于窗格树之外的 pid 会在发送任何信号前被 `process_out_of_scope` 拒绝，因此该方法无法终止任意进程。`force` 默认为 `false`（Unix 上发送 SIGTERM）；`true` 发送 SIGKILL，Windows 上始终是强制终止。响应为 `type: "ok"`；信号发送失败时返回 `terminate_failed`。
