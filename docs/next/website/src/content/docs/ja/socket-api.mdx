...
| 領域 | メソッド |
| --- | --- |
| サーバー | `ping`、`server.stop`、`server.reload_config`、`server.agent_manifests`、`server.reload_agent_manifests`、`server.dev_servers` |
...
`server.dev_servers` は、ペインのプロセスツリー内のプロセスが所有する TCP リスナーを列挙します — dev servers ウィンドウの背後にあるのと同じデータです。パラメータはありません。レスポンスは `type: "dev_server_list"` で、`servers` 配列を持ちます。各エントリは `pid`、`name`、`address`/`port` のペアからなる `listeners`、および任意の `command`、`uptime_seconds`、`pane_id`、`workspace_id`、`workspace_name`、`pane_title`、`cwd` を報告します。
...
`process.kill` は、ペインのプロセスツリー内にある間だけ pid を終了させます。スコープはリクエストごとに再計算され、ペインのツリー外の pid はシグナルを送る前に `process_out_of_scope` で拒否されるため、このメソッドで任意のプロセスを終了させることはできません。`force` のデフォルトは `false` で、Unix では SIGTERM を送ります。`true` は SIGKILL を送り、Windows では常に強制終了です。レスポンスは `type: "ok"` で、シグナル送信に失敗した場合は `terminate_failed` を返します。
