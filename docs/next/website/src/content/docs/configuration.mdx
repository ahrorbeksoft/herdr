...
Set `backend` to `"cow"` to create [cow](https://github.com/joeinnes/cow) APFS pastures instead of linked Git worktrees (macOS only, requires the `cow` CLI). Pastures are created under `~/.cow/pastures/<repo>/<branch-slug>` — the `directory` option and custom checkout paths do not apply — and are removed with `cow remove`.
...
Inside Ghostty, `name = "ghostty"` derives Herdr's palette from whatever theme Ghostty is configured with (including `dark:X,light:Y` splits), and `name = "ghostty:<theme name>"` pins a specific Ghostty theme. Every discovered Ghostty theme also appears in the Settings theme list; browsing a theme live-recolors the terminal instantly, and applying it writes Ghostty's `theme =` line and reloads Ghostty.
