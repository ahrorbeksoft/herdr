...
| Area | Methods |
| --- | --- |
| Server | `ping`, `server.stop`, `server.reload_config`, `server.agent_manifests`, `server.reload_agent_manifests`, `server.dev_servers` |
...
`server.dev_servers` lists TCP listeners owned by processes inside pane process trees — the same data behind the dev servers window. It takes no params. The response is `type: "dev_server_list"` with a `servers` array; each entry reports `pid`, `name`, its `listeners` as `address`/`port` pairs, and optional `command`, `uptime_seconds`, `pane_id`, `workspace_id`, `workspace_name`, `pane_title`, and `cwd`.

```json
{"id":"req_dev_servers","method":"server.dev_servers","params":{}}
{"id":"req_kill","method":"process.kill","params":{"pid":4242}}
{"id":"req_kill_force","method":"process.kill","params":{"pid":4242,"force":true}}
```

`process.kill` terminates a pid only while it sits inside a pane's process tree. The scope is recomputed per request, and pids outside pane trees are refused with `process_out_of_scope` before any signal is sent, so the method cannot terminate arbitrary processes. `force` defaults to `false`, which sends SIGTERM on Unix; `true` sends SIGKILL, and Windows always performs a hard kill. The response is `type: "ok"`; a failed signal returns `terminate_failed`.
